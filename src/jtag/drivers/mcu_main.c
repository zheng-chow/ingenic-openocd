...
//			状态        类型   FTMS  次------数  数------据
			...
			if (type == 0x00200000) {
				...
				for (bit_cnt = 0; bit_cnt < (scan_size-1); bit_cnt++) {
					status = 0x30 | ((dout >> bit_cnt) & 1) << 2;					//TMS
					(*(volatile unsigned int *)0x10010340) = status;				//设置TMS TDI,输出CLK0
					(*(volatile unsigned int *)0x10010340) = status | 1;				//设置TMS TDI,输出CLK1
				}
				status = 0x30 | (((dout >> bit_cnt) & 1) << 2) | (1 << 1);				//TMS
				(*(volatile unsigned int *)0x10010340) = status;					//设置TMS TDI,输出CLK0
				(*(volatile unsigned int *)0x10010340) = status | 1;					//设置TMS TDI,输出CLK1
			} else {
				if (type != 0x00100000) {
...
//			状态        类型   FTMS  次------数  数------据
			...
			if (type == 0x00200000) {
				...
				for (bit_cnt = 0; bit_cnt < 31; bit_cnt++) {
					status = 0x30 | (((data >> bit_cnt) & 1) << 2);					//TMS
					(*(volatile unsigned int *)0x10010340) = status;				//设置TMS TDI,输出CLK0
					(*(volatile unsigned int *)0x10010340) = status | 1;				//设置TMS TDI,输出CLK1
				}
				status = 0x30 | (((data >> bit_cnt) & 1) << 2) | (tms_flag ? (1 << 1) : 0);		//TMS
				(*(volatile unsigned int *)0x10010340) = status;					//设置TMS TDI,输出CLK0
				(*(volatile unsigned int *)0x10010340) = status | 1;					//设置TMS TDI,输出CLK1
			} else {
				if (type != 0x00100000) {
					...
					status = 0x30 | (tms_flag ? (1 << 1) : 0);					//TMS
					(*(volatile unsigned int *)0x10010340) = status;				//设置TMS TDI,输出CLK0
					if((*(volatile unsigned int *)0x10010300) & 0x00000008)
						buffer |= 1 << bit_cnt;
					share_data2 = buffer;
